...
bot.action('manage_notifications', (ctx) => {
  ...
  db.all('SELECT * FROM notifications', (err, rows) => {
    if (!rows.length) return ctx.editMessageText('هیچ اعلانی نیست');
...
bot.action(/notes_(.+)/, (ctx) => {
  ...
  db.all("SELECT * FROM notifications WHERE title LIKE ?", [`%${lesson}%`], (err, rows) => {
    if (!rows.length) return ctx.editMessageText('جزوه‌ای یافت نشد');
    rows.forEach(row => {
      if (row.file_type === 'document') ctx.replyWithDocument(row.file_id, { caption: row.content });
      else ctx.replyWithPhoto(row.file_id, { caption: row.content });
    });
  });
});

bot.action('support', (ctx) => ctx.editMessageText('پشتیبانی: @FOUKYOUMAN'));
bot.action('back_to_main', (ctx) => ctx.editMessageText('منو اصلی', Markup.inlineKeyboard([ /* main menu */ ])));
